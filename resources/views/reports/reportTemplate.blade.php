...
@section('content')
    <div class="d-flex justify-content-center">
        <div class="card" style="width: 90%; ">
            ...
            @endif
            <div class="card-body">
                <div class="d-flex {{$filtro_aplicado==1?'justify-content-between':($treport=='epm'?'justify-content-between':'justify-content-center')}} align-items-center">
                            ...
                            <div class="row m-2">
                                <div class="col">
                                    ...
                                    <select name="select" id="select_periodo" class="form-control">
                                        ...
                                        @if($treport == 'epmv')
                                            <option value="0">Seleccionar...</option>
                                            <option value="5">Ultimos 7 días</option>
                    ...
                    <div>
                        <a class="btn pdf" data-toggle="tooltip" data-placement="bottom" title="Generar Reporte en PDF" href="{{ route('pdfGenerate',array($treport,'string_info'=>$dd.'-'.$md.'-'.$ad.';'.$dh.'-'.$mh.'-'.$ah.';'.$filtro_aplicado)) }}">
                        ...
                        <a class="btn csv" data-toggle="tooltip" data-placement="bottom" title="Generar archivo CSV"  href="{{ route('csvGenerate',array($treport,'string_info'=>$dd.'-'.$md.'-'.$ad.';'.$dh.'-'.$mh.'-'.$ah.';'.$filtro_aplicado)) }}">
...
<script>
    ...
    selectorPer.addEventListener('change',function(){
       ...
       // debugger;
        ...
        if(opcionSeleccionada == '1'){ 
            ...
            //debugger;
            fechaPrimerDiaSigSemana = obtenerPrimerDiaSemana(fechaObjeto,1);
            ...
            ultimoDiaSemana = new Date(sumaDia(fechaPrimerDiaSigSemana,6,1));
            ...
            fechaHastaField.value = formatDate(fechaObjeto.getFullYear(),fechaObjeto.getMonth(),fechaObjeto.getDate());
            fechaDesdeField.value = formatDate(fechaHasta.getFullYear(),fechaHasta.getMonth(),fechaHasta.getDate());
        }
        else if(opcionSeleccionada == '6'){
            fechaPrimerDiaSemanaAnt = obtenerPrimerDiaSemana(fechaObjeto,-1);
            fechaDesdeField.value = formatDate(fechaPrimerDiaSemanaAnt.getFullYear(),fechaPrimerDiaSemanaAnt.getMonth(),fechaPrimerDiaSemanaAnt.getDate());
            
            ultimoDiaSemana = new Date(sumaDia(fechaPrimerDiaSemanaAnt,6,1));
            fechaHastaField.value = formatDate(ultimoDiaSemana.getFullYear(),ultimoDiaSemana.getMonth(),ultimoDiaSemana.getDate());
        }
    });
    function obtenerPrimerDiaSemana(fecha,modo){
        diaSemana = fecha.getDay(); 
        
        if(modo==1){ 
            diasFaltantes = 7-diaSemana;
            var siguienteFecha = sumaDia(fecha, diasFaltantes+1,1);
        }else if(modo==-1){
            diasFaltantes = 6 + (diaSemana-1);
            var siguienteFecha = sumaDia(fecha, diasFaltantes+1,-1);
        }
    ...
    function sumaDia(fecha,num, modo){
        if(modo==1){
